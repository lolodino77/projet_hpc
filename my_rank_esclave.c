...
if(my_rank == 0){
	...
	while (norm(n, r) > THRESHOLD){ ///////PAS SUR SUR QUELLE CONDITION METTRE
		...
		double old_rz = rz;
	    MPI_Gather(q_part, n_part, MPI_INT, q, recvcount, MPI_INT, root, MPI_COMM_WORLD); /* q <-- A.p */
		MPI_Reduce(MPI_IN_PLACE, &pq, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);// rz = dot(r,z)	
		alpha = old_rz / pq;		
