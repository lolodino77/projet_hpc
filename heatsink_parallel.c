...
int main(int argc, char* argv[])
{
	...
	/* simulation des pas de temps */
		//int premier = rank*(o/p)*n*m;
		//int avant = premier - n*n;
		//int apres = (my_rank + 1)*(o/p)*n*m;
		//int dernier = apres - n*m;
	while (convergence == 0) {
		if(my_rank %2 == 0){
			if(my_rank < p-1) MPI_Send(&T[((my_rank + 1)*(o/p)-1)*n*m], n*m, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD);
			if(my_rank > 0) MPI_Recv(&T[(my_rank*(o/p)-1)*n*m], n*m, MPI_DOUBLE, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			if(my_rank > 0) MPI_Send(&T[my_rank*(o/p)*n*m], n*m, MPI_DOUBLE, my_rank - 1, 0, MPI_COMM_WORLD);		
			if(my_rank < p-1) MPI_Recv(&T[(my_rank + 1)*(o/p)*n*m], n*m, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		...
		//de my_rank*m*n*o/p jusqu'a (my_rank + 1)*m*n*o/p
	//=>//de my_rank*o/p jusqu'a (my_rank + 1)*o/p
		//pointeur sur le premier coeff du k-ième plan
		for(int k = my_rank*o/p;k < (my_rank + 1)*o/p;k++){
			int v = k * n * m;
		...
		if (n_steps % ((int)(1 / dt)) == 0) {
			...
			double max = -INFINITY;
	/* Calcul du max local */
			// printf("de %d jusqu'a %d\n", my_rank*m*n*o/p, (my_rank + 1)*m*n*o/p);
			for(int i = my_rank*m*n*o/p;i < (my_rank + 1)*m*n*o/p;i++){
				// printf("i1 = %d\n", i);
				epsilon += (R[i] + T[i])*(R[i] + T[i]);
				if(R[i] > max){
					max = R[i];
				}
			}
	...
	/* Reduction lie au max local */
			MPI_Reduce(&max, &globalMax, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);		

	/* Reduction lie au epsilon */
			// printf("my_rank = %d, epsilon = %lf\n", my_rank, epsilon);
			// MPI_Allreduce(MPI_IN_PLACE,&delta_T,1,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
			printf("epsilon = %lf\n", epsilon);
			MPI_Allreduce(MPI_IN_PLACE, &epsilon, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
			//MPI_Allreduce(const void* sendbuf, void* recvbuf, int count,MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)


			if(my_rank == 0){
				fprintf(stderr, "T_max = %.1f°C, epsilon = %lf, convergence = %g\n", globalMax - 273.15, epsilon, sqrt(epsilon)/dt);
			}

			// fprintf(stderr, "t = %.1fs ; convergence = %g\n", t, epsilon);
			epsilon = sqrt(epsilon) / dt;
			if(epsilon < 0.1){
				convergence = 1;
				// printf("my_rank : %d, epsilon / dt = %lf / %lf = %lf => ca converge !\n", my_rank, epsilon, dt, res);
			}
		}
		double *tmp = R;
		R = T;
